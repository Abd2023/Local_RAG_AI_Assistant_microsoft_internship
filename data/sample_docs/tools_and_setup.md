...
Microsoft Foundry Local is used to run models on the student device. The default chat model for the course is `phi-4-mini`, and the default embedding model is `qwen3-embedding-0.6b`. On machines with a supported NVIDIA GPU, teams should prefer CUDA GPU variants when available. The Docker/Ollama equivalent chat model is `phi4-mini`.
